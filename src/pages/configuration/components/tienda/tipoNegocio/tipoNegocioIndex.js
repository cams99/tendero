...
import TipoNegocioDrawer from './tipoNegocioDrawer'
import tipoNegocioSrc from './tipoNegocioSrc'
...
function TipoNegocio() {
  ...
  const loadData = () => {
    ...
    tipoNegocioSrc
      .getStoreType('', 5)
      .then(result => {
        setDataSource(getStoreType(result))
  ...
  const moreInfo = () => {
    if (existMoreInfo) {
      ...
      Utils.getNextPage(nextPage)
        .then(response => {
          setDataSource(dataSource.concat(getStoreType(response)))
