...
import tipoUbicacionSrc from './tipoUbicacionSrc'
import TipoUbicacionDrawer from './tipoUbicacionDrawer'
...
function TipoUbicacion() {
  ...
  const loadData = () => {
    ...
    tipoUbicacionSrc
      .getLocationType('', 5)
      .then(result => {
        setDataSource(getLocationType(result))
  ...
  const moreInfo = () => {
    if (existMoreInfo) {
      ...
      Utils.getNextPage(nextPage)
        .then(response => {
          setDataSource(dataSource.concat(getLocationType(response)))
