...
function ProductCategoryDrawer(props) {
  ...
  useEffect(() => {
    ...
    setMakerId(props.edit ? props.editData.product_department_id : null)
  ...
  const onSearchMakers = val => {
    ...
    Utils.getProductDepartamentOptions(val)
      ...
      .catch(err => {
        console.log(err)
        message.error('No se ha podido cargar la informacion de departamento de producto')
      })
  }
    ...
    <>
      <Drawer placement="right" closable={false} onClose={props.closable} visible={props.visible} width={800}>
        <div className={'products-container-combo-tabs'}>
          <Title> {props.edit ? 'Editar categoría de producto' : 'Nueva categoría de producto'} </Title>
          <Divider className={'divider-custom-margins-users'} />
          {/*Fields section*/}
          <Row gutter={16} className={'section-space-field'}>
            <Col xs={12} sm={12} md={12} lg={12}>
              <div className={'title-space-field'}>Nombre</div>
              <Input value={name} onChange={value => setName(value.target.value)} placeholder={'Nombre de categoría de productos'} size={'large'} />
            </Col>
            <Col xs={12} sm={12} md={12} lg={12}>
              <div className={'title-space-field'}>Departamento al que pertenece</div>
              <Select
                loading={loading}
                showSearch
                ...
                placeholder={'Seleccionar departamento de productos'}
