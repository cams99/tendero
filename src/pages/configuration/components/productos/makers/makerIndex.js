...
import makerSrc from './makerSrc'
import Utils from '../../../../../utils/Utils'
import MakersDrawer from './makerDrawer'
...
function Makers() {
  ...
  useEffect(() => {
    ...
    makerSrc
      .getMakers('', 5)
      .then(response => {
        setDataSource(getMakersData(response))
  ...
  const searchText = value => {
    setLoading(true)
    makerSrc.getMakers(value, 5).then(response => {
      setDataSource(getMakersData(response))
      setNextPage(response.data.next_page_url)
      setExistMoreInfo(response.data.next_page_url !== null)
      setLoading(false)
  ...
  const moreInfo = () => {
    if (existMoreInfo) {
      ...
      Utils.getNextPage(nextPage)
        .then(response => {
          setDataSource(dataSource.concat(getMakersData(response)))
          setNextPage(response.data.next_page_url)
          setExistMoreInfo(response.data.next_page_url !== null)
          setLoading(false)
        })
  ...
  return (
    <>
      ...
      <MakersDrawer closable={onClose} visible={visible} edit={editMode} editData={editDataDrawer} cancelButton={onClose} saveButton={onSaveButton} />
    </>
  )
}
