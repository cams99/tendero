...
import productDepartamentSrc from './productDepartamentSrc'
import Utils from '../../../../../utils/Utils'
import ProductDepartamentDrawer from './productDepartamentDrawer'

function ProductDepartament() {
  ...
  useEffect(() => {
    setLoading(true)
    productDepartamentSrc
      .getProductDepartament('', 5)
      .then(response => {
        setDataSource(getProductDepartamentData(response))
  ...
  const getProductDepartamentData = dataSrc => {
  ...
  const searchText = value => {
    setLoading(true)
    productDepartamentSrc.getProductDepartament(value, 5).then(response => {
      setDataSource(getProductDepartamentData(response))
      setNextPage(response.data.next_page_url)
      setExistMoreInfo(response.data.next_page_url !== null)
      setLoading(false)
  ...
  const DeleteRow = data => {
    setLoading(true)
    productDepartamentSrc
      .removeProductDepartament(data.id)
      .then(response => {
  ...
  const moreInfo = () => {
    if (existMoreInfo) {
      ...
      Utils.getNextPage(nextPage)
        .then(response => {
          setDataSource(dataSource.concat(getProductDepartamentData(response)))
          setNextPage(response.data.next_page_url)
          setExistMoreInfo(response.data.next_page_url !== null)
          setLoading(false)
        })
  ...
  const onSaveButton = (method, data, id) => {
    switch (method) {
      case true:
        productDepartamentSrc
          .editProductDepartament(id, data)
          .then(res => {
...
export default ProductDepartament
